...
@interface LZZRefreshTableFooterView (Private)
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		[self setState:LZZOPullRefreshNormal];
...
- (void)refreshLastUpdatedDate {
	...
	if ([_delegate respondsToSelector:@selector(lzzRefreshTableFooterDataSourceLastUpdated:)]) {
		...
		[[NSUserDefaults standardUserDefaults] setObject:_lastUpdatedLabel.text forKey:@"LZZRefreshTableView_LastRefresh"];
...
- (void)setState:(LZZPullRefreshState)aState{
	...
	switch (aState) {
		case LZZOPullRefreshPulling:
			...
			break;
		case LZZOPullRefreshNormal:
			
			if (_state == LZZOPullRefreshPulling) {
				[CATransaction begin];
				[CATransaction setAnimationDuration:FLIP_ANIMATION_DURATION];
...
- (void)lzzRefreshScrollViewDidScroll:(UIScrollView *)scrollView {
	...
	if (_state == LZZOPullRefreshLoading) {
...
//		offset = MIN(offset, 60);
		...
		if (_state == LZZOPullRefreshPulling && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
			[self setState:LZZOPullRefreshNormal];
		} else if (_state == LZZOPullRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight  && !_loading) {
			[self setState:LZZOPullRefreshPulling];
		}
...
- (void)lzzRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
		...
		[self setState:LZZOPullRefreshLoading];
...
//当开发者页面页面刷新完毕调用此方法，[delegate lzzRefreshScrollViewDataSourceDidFinishedLoading: scrollView];
